...
7. **Unified state-aware sensor** — `Sensor.stateAware: boolean` replaces the old `sophistication` enum and `trafficAwareness` flag. When `stateAware=true`, each ray produces two brain inputs `[1-distance, state]` where state encodes how blocking the obstacle is (0=clear, 0.5=caution, 1=stop). Legacy mode uses `[1-distance]` per ray. Total input layer: `stateAware ? rayCount*2+1 : rayCount+1`. `CarBrainAdapter.inputLayerSize(rayCount, stateAware)` calculates this. `brainsCompatible()` validates layer dimensions to reject cross-mode swaps. Old `.car` files without `stateAware` default to `false` (backward compatible). The UI exposes a "State Aware" checkbox (`#tiCarStateAware` / `#carStateAware`) instead of the old `<select>`.
...
9. **State-aware sensors** — Lights update via `TrafficManager` inside `World.draw()`, so AI perception reads the previous frame's state (one-frame lag, acceptable). The `stateAware` boolean gate controls whether traffic controls and other cars are passed to `Sensor.update()`.
