...
- **Segment OSM metadata:** `Segment` carries optional `highwayType`, `name`, `lanes`, `surface`, `maxSpeed`, `ref`, `destination`, `destinationRef`, `bridge`, `layer`, `laneMarkings`, `roundabout`, `nameEn`, `nameHe`, `nameAr`, `nameRu`, `maxspeedType` fields (5th constructor arg, a metadata object). Populated by `Osm.parseRoads()` (localized names from `name:en`/`name:he`/`name:ar`/`name:ru` tags), preserved by `Graph.load()` (restored after endpoint reconstruction), and included in `Graph.hash()` (lane count, name, maxSpeed, ref, destination, destinationRef, nameEn, nameHe, nameAr, nameRu, maxspeedType folded per-char; bridge/layer/laneMarkings/roundabout folded into hFlags). All fields are `undefined` for hand-drawn segments and legacy worlds — road generation defaults to 2 lanes (`world.roadWidth = 100` behavior preserved). `Envelope` exposes a public `get skeleton()` getter so the renderer can read the segment's `highwayType` for per-type fill colors. `oneway: "-1"` (reverse one-way) sets `oneWay=true` AND swaps p1/p2 per-segment so the direction matches traffic flow. `junction=roundabout` implies one-way and stores `roundabout: true`. `maxspeed:type` infers `maxSpeed` from a `MAXSPEED_TYPE_DEFAULTS` lookup table (in `osm.ts`) when the explicit `maxspeed` tag is absent. `*_link` highway types (`motorway_link`, `trunk_link`, `primary_link`, `secondary_link`, `tertiary_link`) are mapped to their parent type's tier rank, fill color, and default lane count (motorway/trunk links → 2, primary/secondary/tertiary links → 1). `unclassified` defaults to 2 lanes and `#BBB` fill.
- **OSM node-marking import:** `Osm.parseRoads()` also returns four placement arrays — `lights`, `crossings`, `stops`, `yields` (each `OsmMarkingPlacement[]` = `{ center, directionVector, width, height? }` — plain math primitives, so the math layer never imports the world-layer marking classes). Built from nodes tagged `highway=traffic_signals` / `crossing` / `stop` / `give_way` respectively; requires the Overpass filter to end with `out body;` for nodes (NOT `out skel;`, which strips node tags). These are all nodes ON the highway ways, so no query change is needed beyond `out body;`. `WorldEditor.parseOsmData()` constructs a `Light`/`Crossing`/`Stop`/`Yield` per placement, `setAnchor`s each, and pushes onto `world.markings` **in place** (the `TrafficManager` holds that array reference and re-reads it for control centers — do NOT reassign `world.markings`). `markingLoader` already handles all four types for save/load. **Lights** are signal HEADS facing oncoming traffic, so they use `placeApproachMarking()` (in `osm.ts`), which resolves facing in priority order: (1) `directedApproach` — the node's `direction` / `traffic_signals:direction` tag (`forward`=approaches from prev node, `backward`=from next), authoritative when present; (2) radial — signals within `SIGNAL_CLUSTER_RADIUS_PX` (400 world-px ≈ 28 m) form one junction and the approach arm is the approach-side neighbour pointing most OUTWARD from the centroid; (3) `throughAxis` — the straight-through road, drawn at the node (isolated signal). When an arm is chosen the light slides UPSTREAM along the road centreline (`center + dir * min(halfWidth, span/2)`) to the stop line, staying centred on the road width. Approach sides come from one-way travel (two-way → both sides; reverse one-way swaps sides). **Stops/give-ways** are DIRECTIONAL painted markings (the "STOP"/"YIELD" text must read for the approaching driver), so they orient via `approachFacingDir()` (in `osm.ts`): (1) the node's `direction` tag; (2) the single one-way approach neighbour (upstream side); (3) two-way roads face AWAY from the junction — the driver approaches the higher-`degree` (more-connected) neighbour, so the sign faces the neighbour most opposite to it (`degree` comes from `nodeRefCount`, how many way-endpoints reference each node); (4) `throughAxis` fallback for mid-block/ambiguous. `approachFacingDir()` returns the vector pointing back toward oncoming traffic, but the stop/yield placement **negates** it so the emitted `directionVector` matches the lane-guide convention that `Stop`/`Yield` `draw()` expects (the shared draw rotates `angle(dv) - π/2`; manual markings pass the lane guide's `directionVector()` directly, which is the OPPOSITE orientation to `approachFacingDir`, so the negation makes an OSM-imported sign render identically to a hand-placed one). **Per-lane stop/yield expansion:** `osm.ts` emits ONE stop/give-way seed per node, but a single centred marking can only face one travel direction, so `WorldEditor.parseOsmData()` expands each seed into one marking PER APPROACH LANE via `expandDirectionalMarking()` (`ts/world/osmDirectionalMarkings.ts`) — it picks the approach segment as the incident road whose AXIS is most collinear with the seed direction (`max |dot(segDir, seed)|`; selecting by axis rather than which endpoint is up/downstream keeps the marking on the approach road even where it bends or dead-ends into the junction), builds that segment's per-lane guides with `laneGuidesForSegment()` (extracted from `wgGenerateLaneGuides`, exported from `worldGenerator.ts`), and keeps the approaching lanes: two-way → only lanes on the driver's RIGHT of the road centre (`dot(laneCenter−node, right) > 0` where `right = (−seed.y, seed.x)` is the driver's right under Israel's right-hand traffic), one-way → ALL lanes (all flow into the junction). Every emitted marking faces 180° from the seed travel direction (the seed's `directionVector` negated) so the painted text reads for the approaching driver. Size = `OSM_STOP_YIELD_SIZE_PX` (=`LANE_WIDTH_PX`). A node with no incident approach segment falls back to the single centred seed. `STOP_LINE_SETBACK_PX` (default 0) optionally nudges the marking upstream (against travel). **Crossings** are symmetric zebra lines, oriented across the road via `throughAxis` and drawn at the node. Widths: crossing = full road (`lanes * LANE_WIDTH_PX`), stop/yield = half. Tunables: `SIGNAL_CLUSTER_RADIUS_PX` and the upstream offset (road half-width). **Parking** is imported differently — it is a WAY-side attribute, not a node, AND it is a road-geometry property, not a marking: `hasParkingSide()` reads `parking:right*` / `parking:left*` / `parking:both*` (and legacy `parking:lane:*`; value `no`/`none` = absent) per way and records `parkingLeft` / `parkingRight` on the `Segment` metadata (reverse one-ways swap the sides). During road generation these flags bake a parking LANE into the collision/asphalt envelope (see "Per-segment road width"), so the road border sits AFTER the parking and training cars don't crash on it. The white "P" glyphs are drawn from the segment metadata by `World.#drawParkingLanes()` (spaced along the parking-lane centre), NOT as standalone `Parking` markings. The manual `ParkingEditor` remains a legacy single-spot tool (one `roadWidth/2` square on a lane guide) and does NOT widen the envelope.
- **Per-segment road width:** `LANE_WIDTH_PX = 50` (in `ts/math/worldUnits.ts`) drives per-segment road width: `getSegmentRoadWidth(segment) = (segment.lanes ?? 2) * LANE_WIDTH_PX`. `WorldGenerator.generateRoads`, `wgGenerateLaneGuides`, and `wgGenerateBuildings` all use the per-segment width instead of the uniform `world.roadWidth`. At 14px/m, 50px ≈ 3.57m per lane (real-world lane width). Multi-lane roads (3+ lanes) draw N-1 internal lane dividers in `World.#drawMultiLaneDividers`; 1-2 lane roads use the original `#drawSimpleLaneMarkings` logic. **Parking lanes:** `getSegmentEnvelopeGeometry(seg)` (in `worldGenerator.ts`) returns `{ width, offset }` for the collision/asphalt envelope: each `parkingLeft`/`parkingRight` flag adds one `PARKING_LANE_WIDTH_PX` (=`LANE_WIDTH_PX/2`) to the band. Because `Envelope` is symmetric, one-sided parking is represented by widening the band AND shifting it via the new `Envelope` `lateralOffset` param (right-only → `+P/2`, left-only → `−P/2`, both → `0`), so the driving lanes stay put (lane guides still use `segment.lanes`) and the border lands past the parking lane. `Envelope`'s `lateralOffset` shifts only the generated polygon — `env.skeleton` still points at the real segment so the renderer's `skeleton.highwayType`/`.bridge`/tier-rank reads are unaffected. `wgGenerateBuildings` also uses the widened width so buildings don't overlap the parking lane.
- **Per-lane lane guides:** `wgGenerateLaneGuides` generates one guide per lane at each lane center, replacing the half-width envelope union which placed guides at ±¼-road-width (only correct for 2-lane roads). Direction convention matches the `-angle(dv)+π/2` heading formula (cars face OPPOSITE to dv): two-way roads alternate direction per lane (even-indexed lanes point p1→p2, odd point p2→p1); one-way roads have ALL lanes pointing p2→p1 so cars face forward (p1→p2, with traffic).
- **Tree placement spatial index (`OwnerGrid`):** `wgGenerateTrees` (`ts/world/generation/worldGenerator.ts`) previously tested every candidate point against **all** illegal polygons (buildings + road envelopes) via `Polygon.distanceToPoint`/`containsPoint` (O(candidates × polygons × edges)) — this froze the browser for minutes on large imported OSM maps ("Regenerate items"). `OwnerGrid` is a private, generation-only uniform hash grid (same `Int32Array` stamp-dedup as `SpatialHashGrid`) that maps cells → **owner ids** rather than segments: illegal-polygon edges are bucketed by polygon index, placed trees by tree index. Each candidate queries `radius = treeSize * 2` (the widest check) and runs exact tests on only the nearby owners. It is **not** `SpatialHashGrid` because that grid returns raw `[Point, Point]` edges (no owner mapping, can't index points). Behaviour-preserving: any point relevant to the inside/near/close checks has an edge within `treeSize * 2`. Documented in [docs/Math.md](docs/Math.md) "Owner Grid — tree placement". Building placement (`wgGenerateBuildings`) was not the bottleneck and is unchanged.
...
- **Traffic-manager control-center caching (perf):** `TrafficManager.#getCrossroads()` is `O(points × segments)` (calls `Segment.includes`→`Point.equals` for every point×segment pair) — running it every frame froze the app on large OSM imports (~76% of frame time). It is now cached by `Graph.hash()`, and control centers are rebuilt only when a cheap signature (graph hash + `Light` count + `Light` positions) changes via `#refreshControlCenters(graphHash)`. `update(graphHash?)` takes the frame hash from `World.draw()`. Overrides don't change the signature, so cycling/pausing a light never triggers an `O(n²)` rebuild.
- **Single `Graph.hash()` per frame (perf):** `Graph.hash()` is `O(n × metadata)` and was recomputed independently by every consumer each frame (traffic manager, `World.#getDrawOrderedEnvelopes`, and all four `WorldSignageRenderer` getters — `#getSignage`/`#getOneWayArrows`/`#getRoadShields`/`#getExitSigns`), ~5+ redundant passes. `World.draw()` now computes it once and threads it: `trafficManager.update(hash)`, `#getDrawOrderedEnvelopes(hash)`, and `signageRenderer.setFrameHash(hash)` (getters read `#frameHash ?? graph.hash()`). The frame hash is also stored in `World.#frameGraphHash` so internal envelope readers (`#drawBridgeShadows`, `#drawBridgeDetails`) reuse it via `#getDrawOrderedEnvelopes()`'s default arg (`= this.#frameGraphHash ?? this.graph.hash()`) instead of triggering a fresh hash — an easy regression to reintroduce if a new internal caller omits the arg. `WorldDrawOptions.graphHash` lets a caller that already computed the hash pass it in — `WorldEditor.draw()` reuses its change-detection hash so the editor computes exactly one hash per frame. Cache-invalidation semantics are unchanged (still keyed by the same hash string).
- **Viewport culling of road rendering (perf):** On large OSM maps `World.draw()` drew every segment/envelope/marking each frame regardless of visibility (~50% of frame time in `drawSegment`/`drawEnvelope`/`stroke`/`fill`/`setLineDash`). `Viewport.getVisibleBounds(margin?)` returns the visible world-space AABB (`VisibleWorldRect`, mirrors the `reset()` transform: screen center → `-getOffset()`, one canvas px spans `zoom` world units). Callers pass it as `WorldDrawOptions.screenBounds`; `World.draw()` skips any road envelope, road border, lane marking, parking glyph, bridge polygon, or marking whose AABB (expanded by `WORLD_CULL_MARGIN_PX = 300`) lies off-screen, via `#polygonInView` / `#segmentInView` / `#pointInView`. When `screenBounds` is omitted (e.g. tests, off-screen renders) nothing is culled — behavior-preserving fallback. Wired in all five draw sites (world editor + training world-mode + traffic + human-backprop + race). Buildings/trees keep their existing `renderRadius` distance cull (orthogonal). `Viewport.getVisibleBounds` is unit-tested in `tests/unit/viewport/viewport.test.ts`.
...
- **Experience replay in `CarLearningManager`:** The human-backprop learning subsystem lives in `ts/car/brain/carLearningManager.ts` (`CarLearningManager`), not `Car`. It owns a ring buffer `#replayBuffer` (max 4096 entries) storing `{inputs, targets, isTurn}`, class-balanced batch sampling (`#trainBatch` samples a **balanced** 50/50 batch of turn vs straight entries, size `#batchSize = 16`), per-output learning rates (`[lr, lr*1.5, lr*1.5, lr]`), and decision-point detection via `#prevControlState` (when control state changes, the frame is trained 3 extra iterations). It trains exclusively through `CarBrainAdapter` (never imports `NeuralNetwork`). `Car` holds a `#learning = new CarLearningManager()` instance: `#processBrain` calls `this.#learning.resetFrame()` each frame, `setLastBrainOutput(output)` after inference, and (only when learning from a human) `this.#learning.learn({ brain, inputs, controls })`. `Car`'s public learning API (`get/set learningRate`, `setLearningRate`, `lastBrainOutput`, `brainChangedThisFrame`) is thin delegation to the manager. The manager is inert for all non-learning cars (AI/genetic/traffic/race) — `learn` is never called. Unit-tested in `tests/unit/car/carLearningManager.test.ts`.
...
- **World editor road panel & inspect mode:** `<world-editor-panel>` (`ts/ui/organisms/worldEditorPanel.ts`, top-right corner, CSS `styles/organisms/_world-editor-panel.css`) is the organism that owns road-drawing metadata. It exposes a **brush state** (`highwayType`, `lanes`, `oneWay`, `separated`, `name`, `nameEn`, `nameHe`, `nameAr`, `nameRu`, `maxSpeed`, `ref`, `bridge`, `laneMarkings`) that `GraphEditor.setBrushState()` consumes to stamp metadata onto hand-drawn `Segment`s (5th constructor arg). The Properties section has a collapsible **Localized names** sub-block with `name:en`/`name:he`/`name:ar`/`name:ru` text inputs (wired into `BrushState`/`SegmentMetadata` like the existing `name` field). The Road Type section also holds a global **Labels** display-language dropdown (Native / EN / HE / AR / RU) that calls `setSignageLanguage()` and fires `setOnSignageLanguageChange()` (wired by `WorldEditor` to a redraw) — this is NOT routed through the brush/metadata flow. Road-type selection auto-sets defaults via `applyRoadTypeDefaults()` (extracted with `defaultLaneCount`/`getRoadFillColor`/`ROAD_TYPES` into the pure-data module `ts/math/roadTypes.ts`). The `O`/`H`/`T` toggles moved from `<shortcuts-toolbar>` into the panel's Path Tools section: their `ShortcutBinding`s carry `hidden: true` so `KeyboardManager` still routes/toggles them but `#rebuild()` filters them out of `setShortcuts()`. `EditorType 'inspect'` (`ts/world/editors/inspectEditor.ts`) lets the user click an existing segment to view/edit its metadata (including localized names) in the panel. In `editor-toolbar`, Inspect 🔍 sits next to Graph 🌐 (own group + separator); the generic Marking 🔲 editor lives with the other marking editors. Panel state is session-only (no persistence) except the signage language (persisted to `localStorage` under `sim:signageLanguage`). The panel wraps its content in `.wep-panel` (class-scoped CSS, `text-align: left`, overriding the global `label { justify-content: center }` base rule).
...
- **Single-lane road markings:** `World.#drawSimpleLaneMarkings` skips all center-line drawing when `laneCount <= 1` (prevents a dashed center line from making a 1-lane road look like 2 lanes). One-way arrows are still drawn via `WorldSignageRenderer.drawOneWayArrows`.
- **Road signage placement:** `ts/world/roadSignage.ts` is a pure-placement module (no canvas/DOM) that computes street-name labels (connected same-name segments grouped into street polylines, labels evenly spaced ~`STREET_LABEL_SPACING_PX` = 1000 px apart, min 1 per street, angles normalized upright, nudged/skipped within `LABEL_SIGN_AVOID_RADIUS_PX` = 100 px of a sign; display name chosen by the **signage display-language preference** — `ts/world/signageLanguage.ts`, values `native | en | he | ar | ru`, default `native`, persisted to `localStorage` under `sim:signageLanguage`; resolver prefers the chosen language then falls back `name` → `nameEn` → `nameHe` → `nameAr` → `nameRu`, grouping by the RESOLVED name), speed-limit signs (only at nodes where incident `maxSpeed` values change, offset `SPEED_SIGN_NODE_OFFSET_PX` = 60 px into each affected segment, plus one fallback sign per isolated uniform-limit zone), road shield signs (segments with `ref` tag grouped by ref into connected walks, badges spaced ~`ROAD_SHIELD_SPACING_PX` = 2000 px apart, colored by `highwayType`), and exit destination signs (green gantry signs on `_link` segments with `destination` tag, placed at the higher-tier endpoint). Rendering lives in the `WorldSignageRenderer` collaborator (`ts/world/worldSignageRenderer.ts`, Layer 2, canvas only in `draw*` methods), which owns the placement caches `#signageCache`, `#shieldCache`, and `#exitSignCache` keyed by `Graph.hash()` (the street-label cache key additionally folds the active `getSignageLanguage()` so a language change invalidates it) and exposes `drawRoadNames`/`drawSpeedLimits`/`drawRoadShields`/`drawExitSigns`/`drawOneWayArrows` (each takes `(ctx, graph, zoom?)` and applies the `MIN_SIGNAGE_ZOOM` early-return guard). `World` holds a `#signageRenderer` instance and delegates from `World.draw()` at the same call order; the pure placement math still lives in `roadSignage.ts`/`oneWayArrows.ts`. `Graph.hash()` folds `Segment.name`/`maxSpeed`/`ref`/`destination`/`destinationRef`/`nameEn`/`nameHe`/`nameAr`/`nameRu`/`maxspeedType` so metadata edits invalidate the signage caches.
- **One-way arrow placement:** `ts/world/oneWayArrows.ts` is a pure-placement module (no canvas/DOM) that computes chain-aware one-way arrow positions using the shared walk helpers in `ts/world/streetWalk.ts` (extracted from `roadSignage.ts` — `WalkPiece`, `sharesEndpoint`, `buildConnectedComponents`, `orderSegmentWalk`). Connected one-way segments are chained into walks and arrows are spaced evenly (~`ONE_WAY_ARROW_SPACING_PX` = 200 px) along the total chain instead of per-segment. Arrows draw as a shaft line + filled triangular head via a single `WorldSignageRenderer.drawOneWayArrows()` pass after lane markings, cached in the renderer's `#oneWayArrowCache` keyed by `Graph.hash()`.
- **Road-type draw order:** `ts/world/roadTiers.ts` centralises a `HIGHWAY_TIER_RANK` table (motorway=9 → track=1, unknown/hand-drawn=0) and exports `sortEnvelopesByTier()` for stable ascending-rank sorting. `World` caches the sorted copy in `#drawOrderCache` (invalidated by `Graph.hash()` and reset in `generate()`). Asphalt fills draw tier-sorted so higher-class roads paint on top at overlaps; white borders, lane markings, one-way arrows, and signage stay globally on top.
- **Bridge shadows & lane-markings skip:** `World.#drawBridgeShadows()` draws a drop-shadow for every envelope where `seg.bridge === true`, offset [4,6]px at `rgba(0,0,0,0.3)`, using the tier-sorted draw order so higher-tier bridges cast shadows on lower-tier roads. All bridge shadow polygons are accumulated into a **single path and filled once** — combining sub-paths in one `fill()` keeps overlaps (e.g. where two connected bridge segments' rounded end-caps meet) at a uniform 30% instead of stacking into a darker circle. Called before road borders but after asphalt fills. `World.#drawBridgeDetails()` likewise paints the concrete deck overlay (`rgba(210,210,200,0.15)`) as a single combined path across all bridge envelopes for the same reason (its per-segment parapets/guardrails/joints are still drawn individually as lines). `World.#drawLaneMarkings()` skips any segment where `seg.laneMarkings === false` (from OSM `lane_markings=no` tag) — no center line, no dividers, no arrows on that segment.
...
| `sim:signageLanguage`                   | Signage display-language preference           |
