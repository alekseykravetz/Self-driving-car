...
- **`trainStep` safety guards:** All weight/bias updates have `!isFinite(delta)` and `!isFinite(effectiveLR)` guards to skip corrupted values, and every updated weight/bias is clamped to `[-10, 10]` to prevent unbounded growth.
