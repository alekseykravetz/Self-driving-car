...
- **Bridge shadows & lane-markings skip:** `World.#drawBridgeShadows()` draws a drop-shadow polygon (offset [4,6]px, `rgba(0,0,0,0.3)`) for every envelope where `seg.bridge === true`, using the tier-sorted draw order so higher-tier bridges cast shadows on lower-tier roads. Called before road borders but after asphalt fills. `World.#drawLaneMarkings()` skips any segment where `seg.laneMarkings === false` (from OSM `lane_markings=no` tag) — no center line, no dividers, no arrows on that segment.
