...
- **Lane guides as center lines:** `wgGenerateLaneGuides` uses each graph segment's center line as a single lane guide (via `new Segment(seg.p1, seg.p2)`). This replaces the old half-width envelope union approach which placed guides at ±¼-road-width — only correct for 2-lane roads. Center-line guides are correct for all lane counts (1, 2, 3, 4, etc.). One-way direction is inherent in the graph segment's `directionVector()` (p1→p2), so no post-processing is needed.
