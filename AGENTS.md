...
- **OSM traffic-signal import:** `Osm.parseRoads()` also returns `lights: OsmLightPlacement[]` (`{ center, directionVector, width }` — plain math primitives, so the math layer never imports the world-layer `Light`). Built from nodes tagged `highway=traffic_signals`; requires the Overpass filter to end with `out body;` for nodes (NOT `out skel;`, which strips node tags). Pedestrian signals (`crossing=traffic_signals`) are ignored. `WorldEditor.parseOsmData()` constructs a `Light` per placement, `setAnchor`s it, and pushes onto `world.markings` **in place** (the `TrafficManager` holds that array reference and re-reads it for control centers — do NOT reassign `world.markings`). Placement math (in `osm.ts`): (1) each signal's road neighbours are flagged as an _approach_ side when one-way travel can reach the node from them (two-way → both sides; reverse one-way swaps sides); (2) signals within `SIGNAL_CLUSTER_RADIUS_PX` (400 world-px ≈ 28 m) form one junction, centroid = junction centre; (3) the approach arm is the approach-side neighbour pointing most OUTWARD from the centroid (radial only _selects_ the arm, not the facing); (4) `directionVector` = that neighbour's real centreline direction so the strip spans across the road; (5) the light slides UPSTREAM along the centreline (`center + roadDir * min(halfWidth, span/2)`) to the stop line, staying centred on the road width. Isolated signals fall back to `throughAxis` (two most-opposite neighbours), drawn at the node. Tunables: `SIGNAL_CLUSTER_RADIUS_PX` and the upstream offset (road half-width).
