...
- **World editor road panel & inspect mode:** `<world-editor-panel>` (`ts/ui/organisms/worldEditorPanel.ts`, top-right corner, CSS `styles/organisms/_world-editor-panel.css`) is the organism that owns road-drawing metadata. It exposes a **brush state** (`highwayType`, `lanes`, `oneWay`, `separated`, `name`, `maxSpeed`, `ref`, `bridge`, `laneMarkings`) that `GraphEditor.setBrushState()` consumes to stamp metadata onto hand-drawn `Segment`s (5th constructor arg). Road-type selection auto-sets defaults via `applyRoadTypeDefaults()` (extracted with `defaultLaneCount`/`getRoadFillColor`/`ROAD_TYPES` into the pure-data module `ts/math/roadTypes.ts`). The `O`/`H`/`T` toggles moved from `<shortcuts-toolbar>` into the panel's Path Tools section: their `ShortcutBinding`s carry `hidden: true` so `KeyboardManager` still routes/toggles them but `#rebuild()` filters them out of `setShortcuts()`. `EditorType 'inspect'` (`ts/world/editors/inspectEditor.ts`) lets the user click an existing segment to view/edit its metadata in the panel. In `editor-toolbar`, Inspect 🔍 sits next to Graph 🌐 (own group + separator); the generic Marking 🔲 editor lives with the other marking editors. Panel state is session-only (no persistence). The panel wraps its content in `.wep-panel` (class-scoped CSS, `text-align: left`, overriding the global `label { justify-content: center }` base rule).
