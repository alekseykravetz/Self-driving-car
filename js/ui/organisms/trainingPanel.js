...
import { createCarsForTraining, getSortedAICars, getTopAICars, getTopCarInfoPool, applyPoolToCars, inferHiddenLayers, } from '../../simulator/training/genetics/poolManager.js';
import { loadPoolFromStorage, savePoolToStorage, discardStoredPool, downloadCarFiles, } from '../../simulator/training/genetics/storageManager.js';
...
export class TrainingPanelElement extends HTMLElement {
    ...
    #cachedStoredPool = null;
    #cachedStoredPoolValid = false;
    ...
    #addEventListeners() {
        ...
        if (this.#poolTableBody) {
            this.#poolTableBody.addEventListener('click', (e) => {
                ...
                if (!row || !this.#poolTableBody.contains(row))
                    return;
    ...
    getCarSettings() {
        const hiddenLayers = this.#carHiddenLayersInput
            ...
            : [...this.hiddenLayers];
        return {
            maxSpeed: this.#readNumericInput(this.#carMaxSpeedInput, DEFAULT_CAR_CONFIG.maxSpeed),
            acceleration: this.#readNumericInput(this.#carAccelerationInput, DEFAULT_CAR_CONFIG.acceleration),
            friction: this.#readNumericInput(this.#carFrictionInput, DEFAULT_CAR_CONFIG.friction),
            width: this.#readNumericInput(this.#carWidthInput, DEFAULT_CAR_CONFIG.width, true),
            height: this.#readNumericInput(this.#carHeightInput, DEFAULT_CAR_CONFIG.height, true),
            hiddenLayers,
            sensor: {
    ...
    setCarSettings(info) {
        ...
        if (this.#carWidthInput)
            this.#carWidthInput.value = String(info.width);
        if (this.#carHeightInput)
            this.#carHeightInput.value = String(info.height);
    ...
    #createCarsWithPool(pool) {
        ...
        console.log(`Generation ${this.iteration} started with ${settings.carCount} cars.`);
    }
    ...
    updateStatsDisplay(alive, dead, frozen, maxDist, bestCarSpeed = 0) {
        ...
        if (this.#statSpeedEl && this.#cachedBestSpeed !== bestCarSpeed) {
            this.#statSpeedEl.textContent = formatKmhFromPxPerFrame(Math.abs(bestCarSpeed));
    ...
    updateBestCarAndPool() {
        ...
        if (++this.#domRefreshCounter >= TrainingPanelElement.DOM_REFRESH_INTERVAL) {
    ...
    #updatePoolTable() {
        ...
        for (let i = 0; i < this.bestPool.length; i++) {
            ...
            if (row.dataset.poolIdx !== String(i))
                row.dataset.poolIdx = String(i);
            ...
            if (rankCell.textContent !== rankText)
    ...
    #updateStatusDots() {
        if (!this.#cachedStoredPoolValid) {
            ...
            this.#cachedStoredPool = safeJsonParse(stored);
            ...
            if (!storedPool || storedPool.length === 0) {
                ...
                if (matches) {
                    ...
                    if (storedInfo.acceleration !== current.acceleration)
                        diffs.push(`acc:${storedInfo.acceleration}→${current.acceleration}`);
                    ...
                    if (storedInfo.sensor.rayCount !== current.sensor.rayCount)
                        diffs.push(`rays:${storedInfo.sensor.rayCount}→${current.sensor.rayCount}`);
                    if (storedInfo.sensor.rayLength !== current.sensor.rayLength)
                        diffs.push(`len:${storedInfo.sensor.rayLength}→${current.sensor.rayLength}`);
                    if (Math.abs(storedInfo.sensor.raySpread - current.sensor.raySpread) >
                        1e-2)
                        diffs.push(`spread:${storedInfo.sensor.raySpread.toFixed(2)}→${current.sensor.raySpread.toFixed(2)}`);
                    if (storedInfo.sensor.rayOffset !== current.sensor.rayOffset)
                        diffs.push(`off:${storedInfo.sensor.rayOffset}→${current.sensor.rayOffset}`);
                    const sHL = (storedInfo.hiddenLayers ?? [6]).join(',');
                    const cHL = (current.hiddenLayers ?? [6]).join(',');
                    if (sHL !== cHL)
                        diffs.push(`hl:[${sHL}]→[${cHL}]`);
