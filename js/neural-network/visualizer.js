...
export class NetworkVisualizer {
    ...
    #buildLayout(ctx, network) {
        ...
        const rowValues = [levels[0].inputs];
        const rowBiases = [null];
        ...
        const nodeX = (count, idx) => lerp(left, left + width, count === 1 ? 0.5 : idx / (count - 1));
    ...
    // ---------------------------------------------------------------------------
    #drawEdges(ctx, layout, time, focusedEdges, hasFocus) {
        const hoveredEdge = this.#hover?.kind === 'connection' ? this.#hover.index : -1;
        for (let e = 0; e < layout.edges.length; e++) {
            const edge = layout.edges[e];
     ...
     */
    #drawSignalParticles(ctx, edge, edgeIndex, time, focused) {
        ...
        for (let k = 0; k < count; k++) {
            ...
            ctx.fillStyle = _a.#color(edge.signal, 0.6 + 0.4 * mag);
            ctx.fill();
        }
    }
    #drawNeurons(ctx, layout, focusedNeuron, hasFocus) {
        for (let n = 0; n < layout.neurons.length; n++) {
            ...
            const coreAlpha = (dim ? 0.4 : 1) * _a.#alpha(node.value);
            const grad = ctx.createRadialGradient(node.x, node.y, 0, node.x, node.y, node.r * 0.85);
            grad.addColorStop(0, _a.#color(node.value, coreAlpha));
            grad.addColorStop(1, _a.#color(node.value, 0));
            ctx.beginPath();
            ...
            if (node.bias !== null) {
                ...
                ctx.setLineDash([3, 3]);
                ctx.strokeStyle = _a.#color(node.bias, dim ? 0.15 : _a.#alpha(node.bias));
                ctx.stroke();
                ctx.setLineDash([]);
            }
            ...
            if (node.arrow && !dim) {
                _a.#drawArrow(ctx, node.x, node.y, node.arrow, node.r);
            }
     ...
     */
    static #drawArrow(ctx, x, y, dir, nodeRadius) {
    ...
    #drawNeuronValue(ctx, node) {
        ...
        if (rightX + chipW <= ctx.canvas.width - 4) {
            _a.#drawLabelChip(ctx, lines, rightX, node.y, 'left');
        }
        else {
            _a.#drawLabelChip(ctx, lines, node.x - node.r - 6, node.y, 'right');
        }
    }
    /**
     ...
     * node.
     */
    #drawWeightLabels(ctx, layout, focusedEdges, hoveredNeuron) {
        ...
        for (const e of focusedEdges) {
            ...
            _a.#drawLabelChip(ctx, [edge.weight.toFixed(2)], x, y, 'center');
        }
    }
    // ---------------------------------------------------------------------------
    ...
    #drawLegend(ctx) {
        ...
        grad.addColorStop(1, _a.#color(1, 1));
        ctx.fillStyle = grad;
    ...
    #drawToggleButton(ctx, x, y) {
        const w = 68;
        const h = 18;
        this.#toggleRect = { x, y, w, h };
        ctx.beginPath();
        _a.#roundRect(ctx, x, y, w, h, 4);
    ...
    #drawConnectionTooltip(ctx, edge) {
        const lines = [
        ...
        _a.#drawTooltip(ctx, lines, this.#mouseX + 14, this.#mouseY + 14);
    }
    // ---------------------------------------------------------------------------
    // Hit-testing
    // ---------------------------------------------------------------------------
    ...
    static #pointSegmentDistance(px, py, x1, y1, x2, y2) {
        const dx = x2 - x1;
        const dy = y2 - y1;
    ...
    static #drawLabelChip(ctx, lines, x, y, align) {
        ctx.font = '10px Arial';
        const padX = 4;
        const padY = 3;
        const lineH = 12;
    ...
    static #drawTooltip(ctx, lines, x, y) {
        ...
        let bx = x;
        let by = y;
        if (bx + w > ctx.canvas.width)
            bx = ctx.canvas.width - w - 4;
        if (by + h > ctx.canvas.height)
            by = ctx.canvas.height - h - 4;
        if (bx < 4)
            bx = 4;
        if (by < 4)
            by = 4;
        ctx.beginPath();
        _a.#roundRect(ctx, bx, by, w, h, 6);
