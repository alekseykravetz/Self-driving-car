...
export class Osm {
    ...
    static parseRoads(data) {
        ...
        const height = deltaLat * METERS_PER_DEGREE_LATITUDE * WORLD_PIXELS_PER_METER;
        ...
        for (const node of nodes) {
            ...
            const x = deltaLon === 0 ? width / 2 : invLerp(minLon, maxLon, node.lon) * width;
        ...
        for (const way of ways) {
            ...
            for (let i = 1; i < nodeIds.length; i++) {
                ...
                if (prevPoint && currentPoint) {
                    ...
                    const isOneWay = oneWayTag === 'yes' || lanesTag === '1' || isRoundabout;
                    ...
                    console.warn(`Points for segment in way ${way.id} not found (nodes ${nodeIds[i - 1]} -> ${nodeIds[i]})`);
                }
            }
        }
