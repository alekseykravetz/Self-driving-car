...
export class Graph {
    ...
    static load(info) {
        ...
        const segments = info.segments.map((i) => new Segment(points.find((p) => p.equals(i.p1)), points.find((p) => p.equals(i.p2)), i.oneWay, i.separated));
    ...
    getSegmentsLeavingFromPoint(point) {
        return this.segments.filter((segment) => segment.oneWay ? segment.p1.equals(point) : segment.includes(point));
    }
    addSegment(segment) {
