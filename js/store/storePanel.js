...
export class StorePanelElement extends HTMLElement {
    ...
    #initTabs() {
        ...
        tabs.forEach((tab) => {
            tab.addEventListener('click', () => {
                ...
                const target = this.querySelector(`.store-tab-content[data-content="${tab.dataset.tab}"]`);
                if (target)
    ...
    #updateSortIndicators(tab) {
        const content = this.querySelector(`.store-tab-content[data-content="${tab}"]`);
        if (!content)
            return;
      ...
      <tr>
        ...
        <td class="store-actions">
          ...
          <button class="store-action-btn store-delete-btn" data-key="${e.key}" title="Delete">🗑️</button>
            ...
            .join('');
        tbody
            .querySelectorAll('.store-export-btn')
            .forEach((btn) => {
        ...
        tbody
            .querySelectorAll('.store-delete-btn')
            .forEach((btn) => {
            btn.addEventListener('click', () => {
    ...
    #updateTabCounts() {
        ...
        this.#setTabCount('localStorage', `${lsEntries.length}/${StoreManager.getTrackedKeyCount()}`);
    }
