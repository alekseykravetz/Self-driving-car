...
export class Corridor {
    ...
    static fromPath(skeleton, roadWidth, roadRoundness, options = {}) {
        ...
        if (extendEnd && envSegments.length > 0) {
            ...
            envSegments.push(new Segment(lastSeg.p2, add(lastSeg.p2, scale(lastSegDir, roadWidth * 2))));
        }
        const tempEnvelopes = envSegments.map((s) => new Envelope(s, roadWidth, roadRoundness));
        ...
        if (skeleton.length > 0) {
            ...
            if (openEnd) {
                borders = removeCorridorCap(borders, skeleton[skeleton.length - 1].p2, radius);
            }
        }
    ...
    static load(info) {
        const skeleton = info.skeleton.map((s) => new Segment(new Point(s.p1.x, s.p1.y), new Point(s.p2.x, s.p2.y)));
        const borders = info.borders.map((s) => new Segment(new Point(s.p1.x, s.p1.y), new Point(s.p2.x, s.p2.y)));
        return new Corridor(skeleton, borders, info.openStart ?? false, info.openEnd ?? false);
    }
