...
import { translate, angle, lerp2D, perpendicular, dot, subtract, scale, add, getNearestSegment, } from '../../math/utils.js';
...
function findAnchorSegment(graph, anchor) {
    return graph.segments.find((s) => s.p1.equals(anchor.p1) && s.p2.equals(anchor.p2));
}
export class Marking {
    ...
    constructor(center, directionVector, width, height) {
        ...
        this.width = width;
        this.height = height;
        this.support = new Segment(translate(this.center, angle(this.directionVector), this.height / 2), translate(this.center, angle(this.directionVector), -this.height / 2));
    ...
    reanchor(graph) {
        ...
        if (!seg || !graph.segments.includes(seg)) {
            ...
            if (!seg)
                return; // anchored road gone — keep last absolute position
            this.#anchorSegment = seg;
        }
