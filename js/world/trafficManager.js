...
export class TrafficManager {
    ...
    #initializeControlCenters() {
        ...
        for (const light of lights) {
            ...
            const nearestCrossroadPoint = getNearestPoint(light.center, crossroadPoints);
            if (!nearestCrossroadPoint) {
                console.warn('Could not find a near intersection for a light at:', light.center);
                continue; // Skip this light if no intersection is close enough
            }
            let controlCenter = this.controlCenters.find((c) => c.equals(nearestCrossroadPoint));
            if (!controlCenter) {
                // Create a new point object for the control center to avoid modifying graph points
                controlCenter = new Point(nearestCrossroadPoint.x, nearestCrossroadPoint.y);
