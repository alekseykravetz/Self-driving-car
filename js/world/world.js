...
import { Tree, buildTreePrototypes, DEFAULT_TREE_SEED, DEFAULT_TREE_PROTOTYPE_COUNT, } from './items/tree.js';
...
import { add, scale, lerp2D, lerp, normalize, magnitude, rotate, mulberry32, } from '../math/utils.js';
...
export class World {
    ...
    static load(info) {
        ...
        if (decoration) {
            ...
            world.trees = (decoration.trees ?? []).map((inst) => loadTreeInstance(inst, world));
            world.buildings = (decoration.buildings ?? []).map((b) => Building.loadFootprint(b));
        }
        else {
            ...
            world.trees = legacyTrees.map((t) => {
                ...
                return new Tree(new Point(t.center.x, t.center.y), world.treeSize, world.treePrototypes[p], p, type, scale);
            });
    ...
    draw(ctx, options) {
        const { viewPoint, cars = [], bestCar = null, showStartMarkings = true, renderRadius = 1000, carAlpha = 0.2, showCarNames = false, layers: layerOverrides, } = options;
        const layers = {
        ...
        if (renderBuildings.length || renderTrees.length) {
            const items = [...renderBuildings, ...renderTrees].filter((i) => i.base.distanceToPoint(viewPoint) < renderRadius);
    ...
    #drawLaneMarkings(ctx) {
        for (const seg of this.graph.segments) {
            if (seg.oneWay) {
                ...
                if (len < arrowLength * 2)
                    continue;
                const numArrows = Math.max(1, Math.floor(len / arrowSpacing));
                const dirVector = seg.directionVector();
                const dir = magnitude(dirVector) > 0.001 ? normalize(dirVector) : new Point(1, 0);
                ...
                for (let i = 0; i < numArrows; i++) {
                    ...
                    const start1 = add(tip, scale(rotate(arrowBaseDir, arrowAngle), arrowLength));
                    const start2 = add(tip, scale(rotate(arrowBaseDir, -arrowAngle), arrowLength));
