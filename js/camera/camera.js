...
import { extrudePolygons, extrudeTreeShapes, extrudeCarShape, } from './extrusion.js';
export class Camera {
    x;
    y;
    z;
    ...
    #updateFrustumPoints() {
        ...
        this.left = new Point(this.x - this.range * Math.sin(this.angle - Math.PI / 4), this.y - this.range * Math.cos(this.angle - Math.PI / 4));
        this.right = new Point(this.x - this.range * Math.sin(this.angle + Math.PI / 4), this.y - this.range * Math.cos(this.angle + Math.PI / 4));
    ...
    #getPolygons(world, options = {}) {
        const { keyCar, bestCar, cars = [], traffic, showTrees = true, showBuildings = true, } = options;
        // Buildings
        const buildingPolygons = showBuildings
            ? extrudePolygons(this.#filter(world.buildings.map((b) => b.base)), 200)
            : [];
        // Trees
        const treePolygons = showTrees
        ...
        const roadSegments = world.corridors.length
            ...
            : world.roadBorders || [];
        const roadPolygons = extrudePolygons(this.#filter(roadSegments.map((s) => new Polygon([s.p1, s.p2]))), 10);
        ...
        if (traffic && traffic.length > 0) {
            for (const car of traffic) {
                if (!car.polygon || car.polygon.length < 4)
                    continue;
        ...
        if (bestCarSource &&
            ...
            const filteredCarBase = this.#filter([
                new Polygon(bestCarSource.polygon.map((point) => new Point(point.x, point.y))),
            ]);
        ...
        const carShadowBases = this.#filter(cars
            .filter((c) => c !== keyCar && c !== bestCarSource)
            .map((c) => new Polygon(c.polygon.map((point) => new Point(point.x, point.y)))));
    ...
    render(ctx, world, options = {}) {
        ...
        const projectedPolygons = polygons.map((polygon) => new Polygon(polygon.points.map((point) => this.#projectPoint(ctx, point))));
