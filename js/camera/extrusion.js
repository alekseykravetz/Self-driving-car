...
export function extrudeCarShape(polygon, height = 15, wheelRadius = 5) {
    ...
    const [c_frontLeft, c_quarterFrontLeft, c_middleLeft, c_quarterBackLeft, c_backLeft, c_backRight, c_quarterBackRight, c_middleRight, c_quarterFrontRight, c_frontRight,] = ceiling.points;
...
export function extrudeTreeShapes(polygons, height = 200) {
    ...
    for (const polygon of polygons) {
        ...
        const trunkBase = polygon.points.map((p) => new Point(lerp(centroid.x, p.x, trunkRadius), lerp(centroid.y, p.y, trunkRadius), 0));
        ...
        const canopyBase = polygon.points.map((p) => new Point(p.x, p.y, -canopyBaseHeight));
