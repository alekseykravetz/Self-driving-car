...
export class MiniMap {
    ...
    draw(options) {
        const { viewPoint, cars, roadColor = 'white', carColor = 'blue', backgroundColor, viewport, compactScaleIndicator = true, } = options;
        ...
        for (const car of cars) {
            ...
            this.#ctx.arc(car.x, car.y, (car.damaged ? 2 : 3) / this.#scaler, 0, Math.PI * 2);
            this.#ctx.fill();
            this.#ctx.stroke();
        ...
        if (viewport) {
            if (!this.#scaleIndicator) {
                this.#scaleIndicator = new ScaleIndicator(this.#size, this.#size, viewport, {
