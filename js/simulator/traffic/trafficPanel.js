...
function tpConfigHtml(car) {
    ...
    return rows
        .map(([label, value]) => `<div class="cfg-row"><span>${label}</span><b>${value}</b></div>`)
        .join('');
}
export class TrafficPanelElement extends HTMLElement {
    ...
    connectedCallback() {
        ...
        deleteDamagedBtn?.addEventListener('click', () => {
            if (this.#onDeleteDamaged)
                this.#onDeleteDamaged();
        });
    }
    getSelectedCar() {
    ...
    setCars(cars) {
        ...
        const list = this.querySelector('#trafficCarsList');
        const empty = this.querySelector('#trafficCarsEmpty');
        const count = this.querySelector('#trafficCount');
        if (!list)
            return;
        if (count) {
        ...
        if (empty)
            empty.style.display = cars.length ? 'none' : '';
        list.innerHTML = '';
        this.#rows = [];
