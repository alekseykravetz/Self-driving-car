...
export class TrafficSimulator extends SimulatorShell {
    ...
    constructor(gameCanvas, networkCanvas, miniMapCanvas, cameraCanvas, host) {
        ...
        this.gameCanvas.addEventListener('mousemove', (e) => (this.#hoverEvent = e));
        this.gameCanvas.addEventListener('mouseleave', () => (this.#hoverEvent = null));
    ...
    #initToolbar() {
        ...
        this.toolbarPanel.configureSelectors({
            carMode: 'single',
            onWorldSelected: (entry) => this.#loadWorld(entry?.data ?? null),
        });
    }
    ...
    // ── Spawning ─────────────────────────────────────────
    #handleSpawnClick(e) {
        if (e.button !== 0 || !this.#world || !this.viewport)
            return;
    ...
    #headingAt(point) {
        if (!this.#world)
            return 0;
        const segment = getNearestSegment(point, this.#world.graph.segments, SEGMENT_SEARCH_RADIUS);
        if (!segment)
            return 0;
     ...
     */
    #drawSpawnPreview() {
        if (!this.#hoverEvent || !this.#world || !this.viewport)
            return;
        const info = this.toolbarPanel.getSelectedCars()[0] ?? null;
        if (!info)
            return;
        ...
        ctx.lineTo(tipX + (Math.cos(2.618) * fx - Math.sin(2.618) * fy) * head, tipY + (Math.sin(2.618) * fx + Math.cos(2.618) * fy) * head);
