...
export function savePoolToStorage(pool) {
    if (pool.length > 0) {
        localStorage.setItem('bestPool', JSON.stringify(pool));
...
export function saveRaceCars(cars) {
    if (cars.length > 0) {
...
export function downloadCarFiles(selectedCars) {
    ...
    for (const { car, poolPosition } of selectedCars) {
        ...
        const fileName = `${dateStr}_p${poolPosition + 1}` +
