...
export class TrainingInitModalElement extends HTMLElement {
    ...
    #bindEvents() {
        this.querySelector('#tiStartBtn')?.addEventListener('click', () => this.#start());
        this.querySelector('#tiCancelBtn')?.addEventListener('click', () => this.#cancel());
        ...
        this.querySelectorAll('input[name="tiBrainSource"]').forEach((radio) => {
            radio.addEventListener('change', () => {
                if (radio.checked)
                    this.#applySourceLock(radio.value);
            });
        });
    }
    // ── Prefill ──────────────────────────────────────────
    ...
    #fillCarConfig(c) {
        ...
        this.#setValue('#tiCarHiddenLayers', (c.hiddenLayers ?? inferHiddenLayers(c.brain) ?? [6]).join(', '));
    ...
    #refreshSources() {
        this.#storedPool =
            safeJsonParse(localStorage.getItem('bestPool')) ?? [];
        ...
        const initial = poolAvailable
            ...
            : selectedAvailable
                ? 'selected'
                : 'fresh';
        const radio = this.querySelector(`input[name="tiBrainSource"][value="${initial}"]`);
        if (radio)
            radio.checked = true;
    ...
    #setSourceAvailability(source, available) {
        ...
        const radio = this.querySelector(`input[name="tiBrainSource"][value="${source}"]`);
        if (radio)
    ...
    #applySourceLock(source) {
        ...
        if (sourceConfig) {
            ...
            if (note)
                note.textContent = '(locked to brain source)';
        }
        else if (note) {
            note.textContent = '';
        }
    ...
    #start() {
        ...
        if (!options)
            return;
        const source = this.querySelector('input[name="tiBrainSource"]:checked')?.value ?? 'fresh';
        const result = {
    ...
    #readCarConfig() {
        const hidden = this.#parseHiddenLayers(this.querySelector('#tiCarHiddenLayers')?.value ?? '6');
        return {
            maxSpeed: this.#num('#tiCarMaxSpeed', DEFAULT_CAR_CONFIG.maxSpeed),
            acceleration: this.#num('#tiCarAcceleration', DEFAULT_CAR_CONFIG.acceleration),
