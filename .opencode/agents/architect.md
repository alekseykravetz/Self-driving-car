...
- **Layer isolation via imports** — The dependency graph should flow strictly downward:
  ...
  4. Layer 4: Simulation Shells, UI, Entry Points (`ts/simulator/`, `ts/ui/`, `ts/store/`, `ts/race/`, `ts/traffic/`, `ts/viewport/`, `ts/camera/`, `ts/audio/`, `ts/mini-map/`, `ts/world/entry.ts`) — may import any lower layer.
