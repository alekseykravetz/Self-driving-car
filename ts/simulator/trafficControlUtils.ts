...
import { Light, type LightState } from '../world/markings/light.js';
import {
  TrafficControlGrid,
  type TrafficControlEntry,
} from '../math/trafficControlGrid.js';
 ...
 */
export function buildTrafficControls(world: IWorld): TrafficControlEntry[] {
  ...
  for (let i = 0; i < world.markings.length; i++) {
    ...
    if (marking instanceof Light) {
      ...
      entries.push({
        ...
        getState: (): LightState => light.state,
      });
    }
  }
  return entries;
 ...
 */
export function queryTrafficControlsNearCar(
  grid: TrafficControlGrid,
  car: Car,
): SensorTrafficControl[] {
  const MIN_RANGE = 100;
