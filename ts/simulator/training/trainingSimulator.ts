class TrainingSimulator extends SimulatorShell {
  ...
  private borderGrid: SpatialHashGrid = new SpatialHashGrid(150);

  // Simple mode state
  private simpleState: SimpleSimState = new SimpleSimState();

  // Training Manager
  private trainingManager!: TrainingPanelElement;

  // Training-init modal (brain source + settings before training starts).
  private initModal: TrainingInitModalElement | null = null;
  ...
  #initWorldMode(): void {
    this.toolbarPanel.configureSelectors({
      ...
      onWorldSelected: (entry) =>
        this.#initializeSimulator((entry?.data as World) ?? null),
    });
    ...
    this.#initializeSimulator((storeWorld as World | null) ?? null, false);
    this.#openInitModal('entry');
  }
  ...
  #initializeSimulator(worldInfo: World | null, startTraining = true): void {
  ...
  #updateWorld(): void {
    ...
    const corridor = this.world.corridors[0] ?? null;
    const collisionBorders: Segment[] = corridor
      ? corridor.skeleton
