...
export function brainsCompatible(a: unknown, b: unknown): boolean {
  ...
  if (na.levels.length !== nb.levels.length) return false;
  for (let i = 0; i < na.levels.length; i++) {
    if (
      na.levels[i].inputs.length !== nb.levels[i].inputs.length ||
...
export function inferHiddenLayers(brain?: unknown): number[] | null {
  const nn = brain as NeuralNetwork | undefined;
  if (!nn || nn.levels.length < 2) return null;
    ...
    if (brains.length > 0 && cars[i].brain) {
      if (aiIndex < brains.length) {
        if (brainsCompatible(brains[aiIndex], cars[i].brain)) {
          cars[i].brain = NeuralNetwork.clone(brains[aiIndex] as NeuralNetwork);
        }
      } else {
        const mutated = NeuralNetwork.mutateFromPool(
          brains as NeuralNetwork[],
          mutationRate,
        );
