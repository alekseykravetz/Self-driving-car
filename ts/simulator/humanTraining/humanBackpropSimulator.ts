...
import type { Controls } from '../../car/controls/controls.js';
...
import {
  ...
  updateSimpleTraffic,
  updateSimpleCars,
} from '../training/modes/simpleModeBehavior.js';
...
export class HumanBackpropSimulator extends SimulatorShell {
  ...
  constructor(
    ...
    ) as HumanTrainingPanelElement;
    this.#configModal = document.querySelector(
      'human-training-config-modal',
    ) as HumanTrainingConfigModalElement;
  ...
  #initMode(): void {
    if (this.#mode === 'world') {
      ...
      const simpleWorld = new SimpleWorld(
        ...
        SIMPLE_MODE_CONFIG.simpleRoadWidth,
      );
  ...
  #initWorld(worldInfo: World | null): void {
    this.world = worldInfo ? World.load(worldInfo) : new World(new Graph());
    this.viewport = new Viewport(
      this.gameCanvas,
      this.world.zoom ?? 1,
      this.world.offset ?? new Point(0, 0),
    );
    this.viewport.setMode(this.toolbarPanel.viewportMode);
    this.miniMap = new MiniMap(
      this.miniMapCanvas,
  ...
  protected draw(time: number): void {
    ...
    // before the car/world are ready or while the config modal is open on an
    // idle/paused first load (must run before the #modalOpen early-return).
    this.resizeLayout();
    ...
    if (this.layoutToolbar.showCameraView && this.camera) {
      this.camera.render(this.cameraCtx, this.world, {
        ...
        traffic:
          this.#mode === 'simple' ? this.#simpleState.traffic : undefined,
      });
    }
  }
