...
function spFormatSize(chars: number): string {
...
class StorePanelElement extends HTMLElement {
  ...
  #initTabs(): void {
    const tabs = this.querySelectorAll<HTMLButtonElement>('.store-tab');
    tabs.forEach((tab) => {
      tab.addEventListener('click', () => {
        ...
        const contents =
          this.querySelectorAll<HTMLElement>('.store-tab-content');
  ...
  #initSorting(): void {
    this.querySelectorAll<HTMLTableCellElement>('th.sortable').forEach((th) => {
      th.addEventListener('click', () => {
        const tab =
          th.closest<HTMLElement>('.store-tab-content')?.dataset.content;
  ...
  #updateSortIndicators(tab: string): void {
  ...
  #renderWorlds(): void {
    ...
    const empty = this.querySelector<HTMLElement>('#storeWorldsEmpty')!;
  ...
  #renderCars(): void {
    ...
    const empty = this.querySelector<HTMLElement>('#storeCarsEmpty')!;
  ...
  #renderLocalStorage(): void {
    ...
    const empty = this.querySelector<HTMLElement>('#storeLocalStorageEmpty')!;
    ...
    tbody
      .querySelectorAll<HTMLButtonElement>('.store-export-btn')
      .forEach((btn) => {
    ...
    tbody
      .querySelectorAll<HTMLButtonElement>('.store-delete-btn')
      .forEach((btn) => {
        btn.addEventListener('click', () => {
  ...
  #setTabCount(tab: string, text: string): void {
    const span = this.querySelector<HTMLElement>(
      `.store-tab-count[data-count="${tab}"]`,
    );
