class Simulator {
  ...
  private traffic: Car[] = [];
  private lastGeneratedTrafficY: number = -700;
  private simpleViewY: number = 0;
  private simpleUpdateCycle: number = 0;
  private trafficIdleStart: Map<Car, number> = new Map();

  // Panel elements
  private topControlsPanel: TopControlsPanelElement;
  private viewControlsPanel: ViewControlsPanelElement;

  // Layout constants
  private readonly CONTROL_PANEL_WIDTH = 200;
  private readonly NETWORK_PANEL_WIDTH = 300;
  private readonly SMALL_VIEW_WIDTH = 300;

  // Loop control
  private animationFrameId: number = -1;

  // Training Manager
  private trainingManager!: TrainingManagerPanelElement;
  ...
  constructor(
    ...
    this.viewControlsPanel = document.querySelector(
      'view-controls-panel',
    ) as ViewControlsPanelElement;
    ...
    this.trainingManager.configure({
      ...
      onCarsCreated: () => {
        ...
        this.traffic = generateInitialTraffic(
          (lane) => (this.world as SimpleWorld).getLaneCenter(lane),
          startInfo.angle,
        );
  ...
  #updateSimpleTraffic(bestCar: Car, simpleWorld: SimpleWorld): void {
    ...
    // Update traffic
    const trafficBorders: Point[][] = this.roadBorders!;
    for (let i = 0; i < this.traffic.length; i++) {
  ...
  #updateSimpleCars(
    ...
    for (let i = 0; i < cars.length; i++) {
      ...
      if (car.damaged) {
        ...
        const nearbyPolygons: Point[][] = [...this.roadBorders!];
  ...
  #handlePauseToggle(paused: boolean): void {
    if (paused) {
  ...
  #drawNetworkVisualizer(time: number, bestCar: Car): void {
